...
public class FindMedianSortedArrays {
    ...
    private int getKth(int[] nums1, int start1, int end1, int[] nums2, int start2, int end2, int k) {
        ...
        //就是如果len2长度小于len1，把getKth()中参数互换位置，即原来的len2就变成了len1，即len1，永远比len2小
        ...
        //为了防止数组长度小于 k/2,每次比较都会从当前数组所剩长度和k/2作比较，取其中的小的(如果取大的，数组就会越界)，因为是索引获得的长度需要 -1
        ...
        //如果nums1[i] <= nums2[j]，表示nums2数组中包含i索引，之前的元素，逻辑上全部淘汰，即下次从i+1开始。
        //而k则变为k - (i - start1 + 1)，即减去逻辑上排出的元素的个数(要加1，因为索引相减，相对于实际排除的时要少一个的)
        if (nums1[i] > nums2[j]) {
